...
bool CTicks::NeedsLocalAnimationFix()
{
	// The custom local-animation path only exists for features that manipulate the local model's
	// animation/yaw (anti-aim, fake angle) or hold back packets (fakelag, doubletap tick shifting).
	return F::AntiAim.AntiAimOn()
		|| Vars::Fakelag::Fakelag.Value
		|| m_bDoubletap || m_bWarp || m_bRecharge || m_bSpeedhack
		|| m_iShiftedTicks != 0 || m_iShiftedGoal != 0;
}
...
{
	// Tickbase exploits removed (doubletap / warp / speedhack shift). The old shift loop re-entered
	// CreateMove to re-run engine prediction at a shifted tickbase, which crashed deep inside
	Reset();
	m_bAntiWarp = m_bTimingUnsure = false;
...
{
	// Doubletap()/AntiWarp() arming removed (tickbase exploits). m_bDoubletap/m_bWarp/m_bAntiWarp stay
	// false, so ManagePacket() takes the clean (PSilent-only) choke path below.
