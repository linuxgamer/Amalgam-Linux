...
struct TickRecord
{
	...
	bool m_bPredicted = false; // true if generated by local swing prediction (future record)
...
public:
	...
	// bDistance: sort by distance instead of time proximity
	// flTimeMod: optional uniform adjustment applied to record times (legacy behavior)
	// bAllowFuture: allow inclusion of predicted future records (m_bPredicted)
	// flFutureTolerance: max future seconds allowed for predicted records
	std::vector<TickRecord*> GetValidRecords(std::vector<TickRecord*>& vRecords, CTFPlayer* pLocal = nullptr, bool bDistance = false, float flTimeMod = 0.f, bool bAllowFuture = false, float flFutureTolerance = 0.f);
