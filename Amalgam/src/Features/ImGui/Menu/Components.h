...
namespace ImGui
{
	...
	inline void RenderBackground(ImU32 uBackground)
	{
		...
		pDrawList->AddRectFilled({ vDrawPos.x, vDrawPos.y }, { vDrawPos.x + vSize.x, vDrawPos.y + vSize.y }, uBackground, 0.f);
	}
	inline void RenderBackground(ImU32 uBackground, ImU32 uBorder, float flInset = H::Draw.Scale())
	{
		...
		pDrawList->AddRectFilled({ vDrawPos.x + flInset, vDrawPos.y + flInset }, { vDrawPos.x - flInset + vSize.x, vDrawPos.y - flInset + vSize.y }, uBackground, 0.f);

		flInset += H::Draw.Scale(0.5f) - 0.5f - H::Draw.Scale();
		pDrawList->AddRect({ vDrawPos.x + flInset, vDrawPos.y + flInset }, { vDrawPos.x - flInset + vSize.x, vDrawPos.y - flInset + vSize.y }, uBorder, 0.f, ImDrawFlags_None, H::Draw.Scale());
	}
	inline void RenderTwoToneBackground(float flSize, ImU32 uTitle, ImU32 uBackground, bool bHorizontal = false)
	{
		...
		if (!bHorizontal)
		{
			pDrawList->AddRectFilled({ vDrawPos.x, vDrawPos.y }, { vDrawPos.x + vSize.x, vDrawPos.y + flSize }, uTitle, 0.f, ImDrawFlags_RoundCornersTopLeft | ImDrawFlags_RoundCornersTopRight);
			pDrawList->AddRectFilled({ vDrawPos.x, vDrawPos.y + flSize }, { vDrawPos.x + vSize.x, vDrawPos.y + vSize.y }, uBackground, 0.f, ImDrawFlags_RoundCornersBottomLeft | ImDrawFlags_RoundCornersBottomRight);
		}
		else
		{
			pDrawList->AddRectFilled({ vDrawPos.x, vDrawPos.y }, { vDrawPos.x + flSize, vDrawPos.y + vSize.y }, uTitle, 0.f, ImDrawFlags_RoundCornersTopLeft | ImDrawFlags_RoundCornersBottomLeft);
			pDrawList->AddRectFilled({ vDrawPos.x + flSize, vDrawPos.y }, { vDrawPos.x + vSize.x, vDrawPos.y + vSize.y }, uBackground, 0.f, ImDrawFlags_RoundCornersTopRight | ImDrawFlags_RoundCornersBottomRight);
		}
	}
	inline void RenderTwoToneBackground(float flSize, ImU32 uTitle, ImU32 uBackground, ImU32 uBorder, float flInset = H::Draw.Scale(), bool bVertical = true, bool bTwoToneBorder = true)
	{
		...
		if (bVertical)
		{
			pDrawList->AddRectFilled({ vDrawPos.x + flInset, vDrawPos.y + flInset }, { vDrawPos.x - flInset + vSize.x, vDrawPos.y + flSize }, uTitle, 0.f, ImDrawFlags_RoundCornersTopLeft | ImDrawFlags_RoundCornersTopRight);
			pDrawList->AddRectFilled({ vDrawPos.x + flInset, vDrawPos.y + flSize }, { vDrawPos.x - flInset + vSize.x, vDrawPos.y - flInset + vSize.y }, uBackground, 0.f, ImDrawFlags_RoundCornersBottomLeft | ImDrawFlags_RoundCornersBottomRight);
			if (bTwoToneBorder)
		...
		{
			pDrawList->AddRectFilled({ vDrawPos.x + flInset, vDrawPos.y + flInset }, { vDrawPos.x + flSize, vDrawPos.y - flInset + vSize.y }, uTitle, 0.f, ImDrawFlags_RoundCornersTopLeft | ImDrawFlags_RoundCornersBottomLeft);
			pDrawList->AddRectFilled({ vDrawPos.x + flSize, vDrawPos.y + flInset }, { vDrawPos.x - flInset + vSize.x, vDrawPos.y - flInset + vSize.y }, uBackground, 0.f, ImDrawFlags_RoundCornersTopRight | ImDrawFlags_RoundCornersBottomRight);
			if (bTwoToneBorder)
	...
	inline void FTooltip(const char* sTooltip, bool bCondition = IsItemHovered(), float flWrapWidth = 300, ImFont* pFont = F::Render.FontSmall, ImVec2* pPosOverride = nullptr, ImVec2* pSizeOverride = nullptr)
	{
		if (bCondition)
		{
			...
			pDrawList->AddRect({ vPos.x + flInset, vPos.y + flInset }, { vPos.x - flInset + vText.x, vPos.y - flInset + vText.y }, F::Render.Background2, 0.f, ImDrawFlags_None, H::Draw.Scale());
			...
			pDrawList->AddRectFilled({ vPos.x + flInset, vPos.y + flInset }, { vPos.x - flInset + vText.x, vPos.y - flInset + vText.y }, F::Render.Background1p5, 0.f);
			flInset = 1;
	...
	inline bool FInputText(const char* sLabel, std::string& sText, float flWidth = H::Draw.Scale(150), int iFlags = ImGuiInputTextFlags_None, ImGuiInputTextCallback fCallback = nullptr)
	{
		...
		float flInset = H::Draw.Scale(0.5f) - 0.5f;
		GetWindowDrawList()->AddRect({ vDrawPos.x + flInset, vDrawPos.y + flInset }, { vDrawPos.x - flInset + vSize.x, vDrawPos.y - flInset + vSize.y }, F::Render.Background2, 0.f, ImDrawFlags_None, H::Draw.Scale());
	...
	inline bool FSelectable(const char* sLabel, ImVec4* pColor, float flRounding = 0.f, bool bSelected = false, int iFlags = ImGuiSelectableFlags_None, const ImVec2& vSize = {})
	{
	...
	inline bool FSelectable(const char* sLabel, ImVec4 tColor = { 0.2f, 0.6f, 0.85f, 1.f }, float flRounding = 0.f, bool bSelected = false, int iFlags = ImGuiSelectableFlags_None, const ImVec2& vSize = {})
	{
		return FSelectable(sLabel, &tColor, flRounding, bSelected, iFlags, vSize);
	}

	inline bool FBeginMenu(const char* sLabel, ImVec4* pColor, float flRounding = 0.f, bool bEnabled = true)
	{
		ImDrawList* pDrawList = GetWindowDrawList();
		ImVec2 vDrawPos = GetDrawPos();
		ImVec2 vOriginalPos = GetCursorPos();
	...
	inline bool FBeginMenu(const char* sLabel, ImVec4 tColor = { 0.2f, 0.6f, 0.85f, 1.f }, float flRounding = 0.f, bool bEnabled = true)
	{
	...
	inline bool Section(const char* sLabel, float flPaddingMod = 0.f, float flMinHeight = 28.f, bool bForceHeight = false, uint32_t uHash = 0)
	{
		...
		// Flat style: no two-tone box behind the content, just a bold accent label with a
		// fading gradient underline, then the widgets below it.
		...
		if (sLabel[0] != '#')
		{
			...
			// Gradient underline beneath the label, fading out at both ends.
			ImDrawList* pDL = GetWindowDrawList();
			const ImVec2 vWin = GetWindowPos();
			const float flWinW = GetWindowWidth();
			const float x1 = vWin.x + H::Draw.Scale(1);
			const float x2 = vWin.x + flWinW - H::Draw.Scale(1);
			const float xm = (x1 + x2) * 0.5f;
			const float yTop = vWin.y + GetCursorPosY() + H::Draw.Scale(4);
			const float flLineH = H::Draw.Scale(1);
	...
	// Back-compat alias: the flat label+underline style is now the default Section look.
	inline bool FlatSection(const char* sLabel, float flPaddingMod = 0.f, float flMinHeight = 28.f, bool bForceHeight = false, uint32_t uHash = 0)
	{
		return Section(sLabel, flPaddingMod, flMinHeight, bForceHeight, uHash);
	}
	...
	{	// WOW fuck this
		...
		ImDrawList* pDrawList = GetWindowDrawList();
		bool bChanged = false;
		// Horizontal accent bar (top/bottom) that fades out toward both ends.
		auto DrawBarFadeH = [&](float x1, float y1, float x2, float y2)
			{
				const ImU32 c = F::Render.Accent;
				const ImU32 ct = c & 0x00FFFFFFu;
				const float xm = (x1 + x2) * 0.5f;
				pDrawList->AddRectFilledMultiColor({ x1, y1 }, { xm, y2 }, ct, c, c, ct);
				pDrawList->AddRectFilledMultiColor({ xm, y1 }, { x2, y2 }, c, ct, ct, c);
			};
	...
	inline bool FButton(const char* sLabel, int iFlags = FButtonEnum::None, ImVec2 vSize = { 0, 30 }, int iSizeOffset = 0, ImFont* pFont = nullptr, bool* pHovered = nullptr)
	{
		...
		float flInset = H::Draw.Scale(0.5f) - 0.5f;
		GetWindowDrawList()->AddRect({ vDrawPos.x + flInset, vDrawPos.y + flInset }, { vDrawPos.x - flInset + vSize.x, vDrawPos.y - flInset + vSize.y }, tColor, 0.f, ImDrawFlags_None, H::Draw.Scale());
		if (bWithin)
		{
			if (bClicked)
				tColor.Value.w /= 10;
				...
				tColor.Value.w /= 20;
			flInset = H::Draw.Scale();
			GetWindowDrawList()->AddRectFilled({ vDrawPos.x + flInset, vDrawPos.y + flInset }, { vDrawPos.x - flInset + vSize.x, vDrawPos.y - flInset + vSize.y }, tColor, 0.f);
		}
		PopStyleColor();
	...
	inline bool FToggle(const char* sLabel, bool* pVar, int iFlags = FToggleEnum::None, bool* pHovered = nullptr)
	{
		...
		// Let a swatch/widget drawn over the toggle's right edge (e.g. an inline FColorPicker) take
		// clicks instead of toggling - without this the button claims the whole row's hover region.
		SetNextItemAllowOverlap();
		bool bReturn = Button(std::format("##{}", sLabel).c_str(), vSize);
		if (pHovered)
			*pHovered = IsItemHovered();

		bool bHovered = !Disabled && IsItemHovered();
		...
		// CATALYST-style square checkbox. Unchecked = dark frame box; checked = vertical accent
		// gradient fill (bright top -> dark bottom) with a white checkmark for contrast.
		{
			ImDrawList* pDrawList = GetWindowDrawList();
			float flBox = H::Draw.Scale(11);
			ImVec2 vBoxMin = GetDrawPos() + ImVec2(vOriginalPos.x + H::Draw.Scale(4), vOriginalPos.y + (vSize.y - flBox) / 2);
			ImVec2 vBoxMax = { vBoxMin.x + flBox, vBoxMin.y + flBox };
			if (*pVar)
			{
				ImColor cTop = tColor;
				ImColor cBot = ImColor(tColor.Value.x * 0.45f, tColor.Value.y * 0.45f, tColor.Value.z * 0.45f, tColor.Value.w);
				pDrawList->AddRectFilledMultiColor(vBoxMin, vBoxMax, cTop, cTop, cBot, cBot);
				pDrawList->AddRect(vBoxMin, vBoxMax, GetColorU32(cTop.Value), 0.f, ImDrawFlags_None, H::Draw.Scale());
			}
			else
			{
				pDrawList->AddRectFilled(vBoxMin, vBoxMax, bHovered ? F::Render.Background1p5L : F::Render.Background1p5, 0.f);
				pDrawList->AddRect(vBoxMin, vBoxMax, F::Render.Background2, 0.f, ImDrawFlags_None, H::Draw.Scale());
			}
		}
	...
	inline bool FSlider(const char* sLabel, float* pVar1, float* pVar2, float flMin, float flMax, float flStep = 1.f, const char* fmt = "%g", int iFlags = FSliderEnum::None, bool* pHovered = nullptr)
	{
		...
		if (pVar2)
		{
			...
			{ // CATALYST gradient over the in-range portion (dark -> bright accent)
				ImU32 uBright = tAccent, uDark = ImColor(tAccent.Value.x * 0.5f, tAccent.Value.y * 0.5f, tAccent.Value.z * 0.5f, tAccent.Value.w);
				pDrawList->AddRectFilledMultiColor({ vDrawPos.x + flLowerPos, vDrawPos.y + vMins.y }, { vDrawPos.x + flUpperPos, vDrawPos.y + vMaxs.y }, uDark, uBright, uBright, uDark);
			}
		...
		else
		{
			...
			{ // CATALYST gradient over the filled portion (dark -> bright accent)
				ImU32 uBright = tAccent, uDark = ImColor(tAccent.Value.x * 0.5f, tAccent.Value.y * 0.5f, tAccent.Value.z * 0.5f, tAccent.Value.w);
				pDrawList->AddRectFilledMultiColor({ vDrawPos.x + vMins.x, vDrawPos.y + vMins.y }, { vDrawPos.x + flPos, vDrawPos.y + vMaxs.y }, uDark, uBright, uBright, uDark);
			}
	...
	inline bool ColorPicker(const char* sLabel, Color_t* pColor, bool bTooltip = true, ImVec2 vSize = { H::Draw.Scale(10), H::Draw.Scale(10) })
	{
		ImVec2 vOriginalPos = GetCursorPos();
		if (Disabled)
		{	// lol
	...
	inline bool FColorPicker(const char* sLabel, Color_t* pColor, int iFlags = FColorPickerEnum::None, ImVec2 vOffset = {}, ImVec2 vSize = { H::Draw.Scale(10), H::Draw.Scale(10) }, ImVec2 vIconOffset = {}, bool* pHovered = nullptr)
	{
		if (Transparent || Disabled)
			PushStyleVar(ImGuiStyleVar_Alpha, 0.5f);
		...
		{
			SameLine();
			if (vSize.y <= H::Draw.Scale(16)) // small square swatch next to a checkbox: pin to the
			{                                  // column's right edge and center it so it lines up with
				float flRightEdge = GetCursorPosX() < GetWindowWidth() * 0.65f // the checkbox square
					? GetWindowWidth() / 2.f - GetStyle().WindowPadding.x * 0.5f // right edge of left column
					: GetContentRegionMax().x; // right edge of the full content area
				SetCursorPosX(flRightEdge - vSize.x - H::Draw.Scale(4));
				vIconOffset.y += (H::Draw.Scale(24) - vSize.y) / 2.f;
			}
			else // legacy tall swatch stacked over a dropdown/slider row
			{
		...
		else
		{
			...
			// Advance by the row's max height (like FToggle/FSlider) so an inline swatch never
			// shrinks the row - this is what made toggles "change height" when enabled.
			DebugDummy({ vContentSize.x, GetRowSize(vContentSize.y) });
		}
		if (pHovered && iFlags & FColorPickerEnum::HoverContents)
			*pHovered = IsItemHovered();
