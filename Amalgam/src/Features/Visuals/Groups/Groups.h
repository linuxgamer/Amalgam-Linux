...
Enum(ESP, Name = 1 << 0, Box = 1 << 1, Distance = 1 << 2, Bones = 1 << 3, HealthBar = 1 << 4, HealthText = 1 << 5, UberBar = 1 << 6, UberText = 1 << 7, ClassIcon = 1 << 8, ClassText = 1 << 9, WeaponIcon = 1 << 10, WeaponText = 1 << 11, Priority = 1 << 12, Labels = 1 << 13, Buffs = 1 << 14, Debuffs = 1 << 15, Flags = 1 << 16, LagCompensation = 1 << 17, Ping = 1 << 18, KDR = 1 << 19, Owner = 1 << 20, Level = 1 << 21, AmmoBars = 1 << 22, AmmoText = 1 << 23, IntelReturnTime = 1 << 24, Avatar = 1 << 25)
Enum(Backtrack, Last = 1 << 0, First = 1 << 1, Always = 1 << 2, IgnoreZ = 1 << 3)

// Lazily rasterized flat SVG class-icon surface texture at iPx pixels (defined in ESP.cpp); -1 if unavailable.
int GetSimpleClassIconTex(int iClass, int iPx);

// Fixed ESP sections (replaces the old user-created group list). Order matters:
// GetGroup iterates in reverse so later entries win when an entity matches more than
namespace SectionEnum { enum SectionEnum { Enemies, EnemyBuildings, Teammates, TeammateBuildings, ViewmodelWeapon, ViewmodelArms, Projectiles, Pickups, Intel, Count }; }

struct Group_t
{
	std::string m_sName = "";

	bool m_bEnabled = false; // fixed-section master enable (disabled by default)

	Color_t m_tColor = {}; // glow / chams entity color
	bool m_bTagsOverrideColor = false;
	...
	// Per-element colors (keyed by ESPEnum bit) so every drawable element is individually colorable.
	std::unordered_map<int, Color_t> m_mColors = {};
	Color_t& ElementColor(int iBit)
	{
		auto it = m_mColors.find(iBit);
		if (it == m_mColors.end())
			it = m_mColors.emplace(iBit, Color_t(255, 255, 255, 255)).first;
		return it->second;
	}

	// Per-group box / health-bar style (was previously shared Vars::ESP globals).
	...
	Color_t m_tHealthBarOverhealColor = { 75, 175, 255, 255 };
	float m_flHealthBarWidth = 2.f;
	int m_iHealthBarPosition = 0; // 0 Left, 1 Right, 2 Above, 3 Below (relative to the box)
	int m_iNamePosition = 0; // 0 Above box, 1 Below box
	int m_iClassIconPosition = 0; // 0 Above name, 1 Below box, 2 Left, 3 Right
	float m_flClassIconScale = 1.f; // class icon size multiplier
	bool m_bClassIconDistanceScale = false; // shrink the class icon as the box (distance) shrinks
	bool m_bSimpleClassIcon = false; // draw the flat SVG silhouette class icon instead of the VTF one
	Color_t m_tSimpleClassIconColor = { 255, 255, 255, 255 }; // tint for the simple class icon
	bool m_bSimpleClassIconOutline = false; // black outline around the simple class icon
	bool m_bAvatarOutline = false; // optional black ring around the ESP avatar circle

	Chams_t m_tChams = {};

	Glow_t m_tGlow = {};
	// When set (player/building sections), the glow uses m_tGlowInvisibleColor for entities not visible
	// to the local player (LOS-blocked); m_tColor is then the "visible" color.
	bool m_bGlowVisCheck = false;
	Color_t m_tGlowInvisibleColor = {};
...
public:
	...
	void Init();            // (re)build the fixed set of sections with their hard-coded targeting
	void ReassertTargets(); // lock the fixed targeting/names back onto the sections (after a config load)
