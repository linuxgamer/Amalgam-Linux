...
{
	// CLEAN POV DEMOS - record-time bisection guard. This routine FrameAdvances the local player's
	// animation every send-packet tick with curtime forced to the (future) tickbase time. That client-side
	if (I::EngineClient && I::EngineClient->IsRecordingDemo() && !I::EngineClient->IsPlayingDemo())
		return;

	// VANILLA WHEN IDLE - when no feature needs the custom local-animation path, skip it entirely so the
	// engine's per-frame UpdateClientSideAnimation runs instead (CBaseAnimating_UpdateClientSideAnimation
	if (!F::Ticks.NeedsLocalAnimationFix())
		return;
	...
	void* rcx, int sequence_number, float input_sample_frametime, bool active)
{
	...
	// NOTE: EnginePrediction.Start is called inside F::Ticks.Start (and paired with the single
	...
	F::Ticks.End(pLocal, pCmd);
		...
		// Texture-bug choke must run AFTER PacketManip + Ticks: PacketManip.Run starts with an
		// unconditional *pSendPacket = true, so consuming the choke before it meant the held
		if (I::ClientState->chokedcommands < 14 && F::Misc.ConsumeTextureBugChoke())
			*pSendPacket = false;
		// F::AntiAim.Run removed - antiaim (tickbase/yaw exploit) disabled per request.
		F::NoSpreadHitscan.AskForPlayerPerf();
	F::EnginePrediction.End(pLocal, pCmd);

	F::Misc.EdgeBugPostPrediction(pLocal, pCmd); // Check after prediction started


	AntiCheatCompatibility(pCmd, pSendPacket);
	LocalAnimations(pLocal, pCmd, *pSendPacket);

	// Clean POV demos: the demo cmdinfo's localViewAngles.
	if (Vars::Misc::Game::CleanPOVDemos.Value || G::FakePOVActive)
	{
		// Source of the demo cmdinfo angle: on NORMAL ticks use the final cmd angle.
		const Vec3 vRealView = G::SilentAngles || G::PSilentAngles ? I::EngineClient->GetViewAngles() : pCmd->viewangles;
		// The GetLocalViewAngles hook has no command slot to index by, so it reads this latest copy when the
		// engine asks for the local view while recording the demo.
		G::RealViewAngles = vRealView;
	}

	// Fake POV "Snap view on disable": run AFTER the stash above so it reads the freshly-stashed
	// G::RealViewAngles, and so its hold-frame drop sees the moved aim once the stash picks it up.
	F::FakePOV.CreateMove(pLocal);
